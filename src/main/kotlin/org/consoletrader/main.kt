...
import org.consoletrader.rx.TradedObservable
import org.consoletrader.tasks.binance.ListOrders
...
fun main(args: Array<String>) {
    ...
    when (task) {
        "listorders" -> ListOrders(apiKey, apiSecret).execute()
...
fun checkArgument(args: Array<String>, parameter: String, message: String): String? {
    var paramDefined = args.any { it.startsWith("-$parameter:") }
    if (paramDefined) {
        return args
                .filter { it.startsWith("-$parameter:") }
                .first()
                .substringAfter(':')
    }
...
fun printUsage() {
    println("USAGE:")
    println("-market:[market] -key:[key] secret:[secret] -task:cancelorders")
